...
interface ProductDetailProps {
  ...
  isLoading?: boolean; // NEW: passed from App.tsx to prevent premature "not found"
}
...
const ProductDetail: React.FC<ProductDetailProps> = ({ products, onAddToCart, isLoading = false }) => {
  ...
  // ─── LOADING STATE ───────────────────────────────────────────────────────────
  // While Supabase is still fetching, show a spinner instead of "Product Not Found".
  // This prevents Google from seeing a 404-like page when it crawls product URLs directly.
  if (isLoading) {
    return (
  ...
  // ─── NOT FOUND STATE ─────────────────────────────────────────────────────────
  // Only shown AFTER loading is complete and the product genuinely doesn't exist.
  if (!product) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
