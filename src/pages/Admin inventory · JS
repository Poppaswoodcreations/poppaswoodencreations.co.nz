// Cloudflare Pages Function: POST /api/admin-inventory
// Password-protected, read-only endpoint for the Inventory dashboard.
// Returns products at or below the low-stock threshold (0 or 1 left).
// Mirrors admin-custom-orders.js's auth + rate-limit pattern.
...
export async function onRequest(context) {
  ...
  const requestCheck = await checkLimit(kv, `admin-inventory:req:${ip}`, REQUEST_LIMIT, REQUEST_WINDOW_SECONDS);
  ...
  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
    console.error('admin-inventory: missing Supabase env vars', {
  ...
  if (password !== ADMIN_PASSWORD) {
    const authCheck = await checkLimit(kv, `admin-inventory:auth:${ip}`, AUTH_FAIL_LIMIT, AUTH_FAIL_WINDOW_SECONDS);
  ...
  try {
    if (action === 'low-stock') {
      const res = await fetch(
        `${SUPABASE_URL}/rest/v1/products?stock_quantity=lte.${LOW_STOCK_THRESHOLD}&select=id,name,stock_quantity,in_stock,category&order=stock_quantity.asc,name.asc`,
        { headers }
      );
      if (!res.ok) return json({ error: await res.text() }, 500);
      return json({ success: true, products: await res.json() });
    }
