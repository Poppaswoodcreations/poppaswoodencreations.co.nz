...
export const getBlogContent = (slug: string): React.ReactNode => {
  switch (slug) {
    case 'happy-go-lucky-train':
      return <HappyGoLuckyTrainContent />;
...
const PoppasWoodenCreationsHandmadeWoodenPineTrolleyAndBlocksContent: React.FC = () => (
  ...
  <>
    ...
    <p className="mb-6">
      There's a simple yet profound joy that comes from watching your child play with a toy that's been lovingly crafted by hand. At <strong>Poppa's Wooden Creations</strong>, we believe in preserving the tradition of handmade toys, using only the best native timbers New Zealand has to offer. Today, we're excited to shine the spotlight on one of our most cherished creations: the <strong>Happy-Go-Luck-Train</strong>, handcrafted from stunning New Zealand Pine.
    </p>
    ...
    <p className="mb-6">
      Pine is known for its <strong>fine grain and versatile nature</strong>, making it the perfect choice for crafting intricate toys that withstand the test of time. The Happy-Go-Luck-Train showcases the beautiful grain patterns of this timber, creating a toy that's not only fun to play with but an aesthetically pleasing addition to your child's toy collection.
    </p>
    ...
    <p className="mb-6">
      But it's not just about physical development. The Happy-Go-Luck-Train is a <strong>gateway to a world of imagination</strong>. Whether your child is embarking on a cross-country journey or delivering goods to a bustling city, this toy provides endless scenarios for imaginative play.
    </p>
    ...
    <p className="mb-6">
      Moreover, we use <strong>non-toxic paints and finishes</strong> to ensure that our toys are safe for your little ones. When you purchase a toy from us, you can rest assured that you're getting a product that is eco-friendly and poses no health risks to your child.
    </p>
    ...
    <p className="mb-6">
      So why not introduce your child to the magic of handcrafted wooden toys? With the Happy-Go-Luck-Train, you're not just giving a toy, you're giving a <strong>legacy of quality, craftsmanship, and love</strong>.
    </p>
