        ...
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden lg:flex items-center space-x-6">
            <div className="relative group">
              <button className="text-gray-700 hover:text-amber-600 transition-colors font-medium text-base">
                Shop
              </button>
              <div className="absolute left-0 mt-2 w-48 bg-white shadow-lg rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                {productCategories.map((category) => (
                  <a key={category.slug} href={`/${category.slug}`} onClick={(e) => { e.preventDefault(); handleCategoryClick(category.slug); }} className="block px-4 py-2 text-gray-700 hover:bg-amber-50 hover:text-amber-600 transition-colors">{category.name}</a>
                ))}
              </div>
            </div>
            <a href="/about" onClick={(e) => { e.preventDefault(); handleCategoryClick('about'); }} className="text-gray-700 hover:text-amber-600 transition-colors font-medium text-base">About</a>
            <a href="/blog" onClick={(e) => { e.preventDefault(); handleCategoryClick('blog'); }} className="text-gray-700 hover:text-amber-600 transition-colors font-medium text-base">Blog</a>
            <a href="/contact" onClick={(e) => { e.preventDefault(); handleCategoryClick('contact'); }} className="text-gray-700 hover:text-amber-600 transition-colors font-medium text-base">Contact</a>
          </nav>
        ...
        {isMenuOpen && (
          <div className="lg:hidden border-t border-gray-200 py-4 bg-white">
            <div className="space-y-2">
              <a href="/" onClick={(e) => { e.preventDefault(); handleCategoryClick('home'); }} className="block w-full text-left px-4 py-2 text-gray-700 hover:bg-amber-50 hover:text-amber-600 transition-colors">Home</a>
              <div className="px-4 py-2">
                <div className="font-medium text-gray-900 mb-2">Shop by Category</div>
                <div className="pl-4 space-y-1">
                  {productCategories.map((category) => (
                    <a key={category.slug} href={`/${category.slug}`} onClick={(e) => { e.preventDefault(); handleCategoryClick(category.slug); }} className="block w-full text-left py-2 text-gray-600 hover:text-amber-600 transition-colors">{category.name}</a>
                  ))}
                </div>
              </div>
              ...
              <a href="/reviews" onClick={(e) => { e.preventDefault(); handleCategoryClick('reviews'); }} className="block w-full text-left px-4 py-2 text-gray-700 hover:bg-amber-50 hover:text-amber-600 transition-colors">Reviews</a>
              <a href="/shipping" onClick={(e) => { e.preventDefault(); handleCategoryClick('shipping'); }} className="block w-full text-left px-4 py-2 text-gray-700 hover:bg-amber-50 hover:text-amber-600 transition-colors">Shipping</a>
            </div>
          </div>
        )}
      </div>
          ...
          <div className="bg-white rounded-xl max-w-md w-full p-6">
            ...
            <form onSubmit={handlePasswordSubmit}>
              ...
              <div className="flex space-x-3">
                <button type="submit" className="flex-1 bg-amber-600 text-white py-2 rounded-lg hover:bg-amber-700 transition-colors">Access Admin</button>
