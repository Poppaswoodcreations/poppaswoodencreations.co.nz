// Cloudflare Pages Function: POST /api/admin-custom-orders
// Handles admin reads/writes for custom order requests, server-side.
...
export async function onRequest(context) {
  ...
  const requestCheck = await checkLimit(kv, `admin-custom-orders:req:${ip}`, REQUEST_LIMIT, REQUEST_WINDOW_SECONDS);
  ...
  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
    console.error('admin-custom-orders: missing Supabase env vars', {
  ...
  if (password !== ADMIN_PASSWORD) {
    const authCheck = await checkLimit(kv, `admin-custom-orders:auth:${ip}`, AUTH_FAIL_LIMIT, AUTH_FAIL_WINDOW_SECONDS);
  ...
  try {
    if (action === 'list') {
      const res = await fetch(
        `${SUPABASE_URL}/rest/v1/custom_orders?select=*&order=created_at.desc`,
        { headers }
      );
    ...
    if (action === 'update') {
      ...
      const res = await fetch(`${SUPABASE_URL}/rest/v1/custom_orders?id=eq.${encodeURIComponent(id)}`, {
        method: 'PATCH',
    ...
    console.error('admin-custom-orders error:', error);
