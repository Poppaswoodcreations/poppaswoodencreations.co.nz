// Cloudflare Pages Function: POST /api/admin-reviews
// Handles all admin review reads/writes (list/insert/update-visibility/
// delete/save-reply/delete-reply) server-side.
...
// Same pattern as admin-products.js and admin-orders.js.
...
export async function onRequest(context) {
  ...
  const requestCheck = await checkLimit(kv, `admin-reviews:req:${ip}`, REQUEST_LIMIT, REQUEST_WINDOW_SECONDS);
  ...
  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
    console.error('admin-reviews: missing Supabase env vars', {
  ...
  const { password, action, id, review, replyText } = body || {};

  if (password !== ADMIN_PASSWORD) {
    const authCheck = await checkLimit(kv, `admin-reviews:auth:${ip}`, AUTH_FAIL_LIMIT, AUTH_FAIL_WINDOW_SECONDS);
  ...
  try {
    if (action === 'list') {
      ...
      return json({ success: true, reviews: await res.json() });
    }

    if (action === 'insert') {
      if (!review) return json({ error: 'Missing review' }, 400);
      ...
      if (!res.ok) return json({ error: await res.text() }, 500);
      return json({ success: true, review: await res.json() });
    }

    if (action === 'update-visibility') {
      if (!id || typeof body.is_visible !== 'boolean') {
        return json({ error: 'Invalid id or is_visible' }, 400);
      }
      const res = await fetch(`${SUPABASE_URL}/rest/v1/reviews?id=eq.${encodeURIComponent(id)}`, {
        method: 'PATCH',
        headers: { ...headers, Prefer: 'return=representation' },
        body: JSON.stringify({ is_visible: body.is_visible }),
      });
      if (!res.ok) return json({ error: await res.text() }, 500);
      return json({ success: true, review: await res.json() });
    }
    ...
    if (action === 'save-reply') {
      if (!id || !replyText || !replyText.trim()) {
        return json({ error: 'Missing id or replyText' }, 400);
      }
      const res = await fetch(`${SUPABASE_URL}/rest/v1/reviews?id=eq.${encodeURIComponent(id)}`, {
        method: 'PATCH',
        headers: { ...headers, Prefer: 'return=representation' },
    ...
    if (action === 'delete-reply') {
      ...
      const res = await fetch(`${SUPABASE_URL}/rest/v1/reviews?id=eq.${encodeURIComponent(id)}`, {
        method: 'PATCH',
        headers: { ...headers, Prefer: 'return=representation' },
        body: JSON.stringify({ owner_reply: null, owner_reply_date: null }),
      });
      if (!res.ok) return json({ error: await res.text() }, 500);
      return json({ success: true, review: await res.json() });
    }
